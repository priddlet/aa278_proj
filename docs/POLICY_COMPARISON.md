...
4. **Realistic dynamics:** use **`presentation/tables/filter_predict/`** (constant Q, CV predict). **`gnss_only`** may diverge — geometry-limited, not a steady-state accuracy claim. Hybrid fusion vs XNAV: compare final mean and segment means; do not over-interpret p95 unless NIS/df ≈ 1 (`python scripts/check_nis.py --filter-predict`).

Regenerate: `python scripts/build_presentation_assets.py` (writes both `truth_velocity/` and `filter_predict/`).
